...
const AgentFloorLink = ({ endpoint }: { endpoint: string }) => {
  const floorUrl = `${endpoint.replace(/\/$/, '')}/floor`
  ...
  return (
    <a
      ...
      className="flex h-9 w-full items-center justify-center rounded-xl border border-primary/30 bg-primary/10 px-3 text-xs font-medium uppercase text-primary transition-colors hover:bg-primary/20"
    >
      Open Agent Floor
    </a>
  )
    ...
    >
      ...
      >
        ...
        {isMounted && (
          <>
            <Endpoint />
            <AgentFloorLink endpoint={selectedEndpoint} />
